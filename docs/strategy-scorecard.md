...
| Retune candidate grid made relative to the current value, not fixed absolute values | Found via direct log inspection: `short_leg_delta_range=(0.05, 0.1)` and `stop_loss_pct=0.4` were both showing `mean diff/trade $+0.00, p=1.000` on every run since 2026-09-11 - because those fixed candidates had come to equal the current live values (stop_loss_pct's default was already one of its own two fixed candidates from day one; short_leg_delta_range became one after the 09-11 adoption), silently wasting half the job's exploration on comparing a value to itself, with no way to explore further in a direction that had already proven better. Fixed by computing each parameter's two candidates as a step away from whatever value is currently live, every run. Verified live: every one of the 9 candidates in the next run showed a genuinely different comparison (no more $0.00 self-comparisons), even though none cleared the significance bar that day - the fix is about search correctness, not a guaranteed new adoption. | 2026-09-20 |
